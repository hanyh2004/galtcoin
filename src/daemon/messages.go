...
import (
	...
	"github.com/samoslab/galtcoin/src/daemon/gnet"
	"github.com/samoslab/galtcoin/src/daemon/pex"
	"github.com/samoslab/galtcoin/src/util/iputil"
	"github.com/samoslab/galtcoin/src/util/utc"
)
...
func (gpm *GivePeersMessage) Process(d Daemoner) {
	...
	//logger.Infof("Got these peers via PEX: %s", strings.Join(peers, ", "))
...
func (intro *IntroductionMessage) Handle(mc *gnet.MessageContext, daemon interface{}) error {
	...
	err := func() error {
		...
		stringSlice := strings.Split(mc.Addr, ":")
		if len(stringSlice) == 2 {
			p, _ := strconv.Atoi(stringSlice[1])
			if p != d.DaemonConfig().Port {
				logger.Infof("%s has different port %d. Disconnecting.",
					mc.Addr, p)
				d.Disconnect(mc.Addr, ErrDisconnectInvalidVersion)
				return ErrDisconnectInvalidVersion
			}
		...
		logger.Infof("%s verified for version %d %d", mc.Addr, intro.Version, d.DaemonConfig().Port)
