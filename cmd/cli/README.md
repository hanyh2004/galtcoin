...
The CLI command APIs can be used directly from a Go application, see [Skycoin CLI Godoc](https://godoc.org/github.com/samoslab/galtcoin/src/cli).
...
```bash
$ cd $GOPATH/src/github.com/samoslab/galtcoin/cmd/cli
$ ./install.sh
```
...
```bash
$ PROG=skycoin-cli source $GOPATH/src/github.com/samoslab/galtcoin/cmd/cli/autocomplete/bash_autocomplete
```
...
CLI will connect to skycoin node rpc address `http://127.0.0.1:6869` by default.
You can change the address by setting the `RPC_ADDR` environment variable
with the following command:

```bash
$ export RPC_ADDR=http://127.0.0.1:6869
```
...
ENVIRONMENT VARIABLES:
    RPC_ADDR: Address of RPC node. Must be in scheme://host format. Default "http://127.0.0.1:6869"
...
```json
{
    ...
    "coin": "skycoin",
    "rpc_address": "http://127.0.0.1:6869",
    "use_csrf": false
}
```
