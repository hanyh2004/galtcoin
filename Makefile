...
run:  ## Run the galtcoin node. To add arguments, do 'make ARGS="--foo" run'.
	./run.sh ${ARGS}

run-help: ## Show galtcoin node help
	@go run cmd/samoslab/galtcoin.go --help
...
cover: ## Runs tests on ./src/ with HTML code coverage
	go test -cover -coverprofile=cover.out -coverpkg=github.com/samoslab/galtcoin/... ./src/...
...
install-deps-libc: configure-build ## Install locally dependencies for testing libskycoin
	git clone --recursive https://github.com/galtcoin/Criterion $(BUILD_DIR)/usr/tmp/Criterion
...
format: ## Formats the code. Must have goimports installed (use make install-linters).
	goimports -w -local github.com/samoslab/galtcoin ./cmd
	goimports -w -local github.com/samoslab/galtcoin ./src
	goimports -w -local github.com/samoslab/galtcoin ./lib
